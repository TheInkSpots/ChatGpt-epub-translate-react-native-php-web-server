...
window.onerror = function (msg, url, line, column, err) {
    ...
    document.querySelector(".app .error .error-description").innerHTML = "erred";
...
let App =  function (el) {
    ...
    this.state = {};
    this.doReset();
    console.log('App reader initialized');
...
App.prototype.fatal = function (msg, err, usersFault) {
    ...
    document.querySelector(".app .error").classList.remove("hidden");
    document.querySelector(".app .error .error-title").innerHTML = "Error";
    document.querySelector(".app .error .error-description").innerHTML = usersFault ? "" : "erre.";
...
App.prototype.doReset = function () {
    ...
    this.qs(".book").innerHTML = '<div class="empty-wrapper"><div class="empty"><div class="app-name">ePubViewer</div><div class="message"></div></div></div>';
...
try {
    ...
    document.querySelector(".app .error").classList.remove("hidden");
    document.querySelector(".app .error .error-title").innerHTML = "Error";
    document.querySelector(".app .error .error-description").innerHTML = "erre";
    document.querySelector(".app .error .error-dump").innerHTML = JSON.stringify({
        error: err.toString(),
        stack: err.stack
    });
...
$(  async () => {
    const arr = await new Promise(async (resolve, reject)=> {
        ...
        console.log('ori2:', urlParameters);

        let url = `../../book/bookshelf(${urlParameters.params['bookid']})`;
        ...
        await $.ajax({
            ...
            success: function (response) {
               // alert('settings');
                console.log('test: ', response.base64Str)
                let arr = base64ToArrayBuffer(response.base64Str);
                console.log(arr);
                resolve(arr);
