        ...
        if(str_contains($parameters[0],'(')){
            ...
            if($method === 'delete'){
                ...
                try {
                    ...
                    try {
                        $conn->query("DELETE FROM $tableName WHERE uuid = '$guid'");
                        $this->echoRespone('success','200',"GUID : '$guid' is deleted.");
                    }catch (Exception $e2) {
                        $this->echoRespone('error','804',',SQL failed. exception  > '.$e2->getMessage());
                    }
