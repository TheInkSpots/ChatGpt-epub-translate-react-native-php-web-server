...
const Bookstore = () => {
  ...
  //   //await deleteUser();
  // }
  console.log('bookstore uuid: ', user.idToken);
  //Icon.loadFont();
  ...
  const onBookList: any = async () => {
    await getBook('/api/bookshelf/?$filter=public [eq] 1')
      .then((result) => {
  ...
  const onSearchBook: any = async (word: string) => {
    ...
    if (word !== null && word !== undefined && word !== '') {
      //setBookList([]);
      await getBook(
        `/api/bookshelf/?$filter=public [eq] 1 [and] author [like] '%${word}%'`,
      )
        .then((result) => {
  ...
  return (
    <ContainerView>
      <ContainerScroll>
        <TitleText>Public Library</TitleText>
        <SearchInput
